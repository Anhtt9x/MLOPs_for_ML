...
class ModelTrainer:
    ...
    def get_model_object_report(self, train:np.array, test:np.array) -> Tuple[object,object]:
        try:
            ...
            metric_artifact = ClassificationMetricArtifact(f1_score=f1,precision_score=precision_score,
                                                           recall_score=recall)
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        logging.info('Entered initiate model trainer method of ModelTrainer class')

        try:
            train_arr = load_numpy_array_data(file_path=self.data_transformation_artifact.transformed_train_file_path)
            test_arr = load_numpy_array_data(file_path=self.data_transformation_artifact.transformed_test_file_path)

            best_model_detail, metric_artifact = self.get_model_object_report(train=train_arr,
                                                                              test=test_arr)
