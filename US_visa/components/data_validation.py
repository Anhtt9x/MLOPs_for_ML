...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe:DataFrame) ->bool:
        try:
            ...
            logging.info(f"Number of columns are matched -> {status}")
            return status
        except Exception as e:
            raise USvisaException(e,sys)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                validation_error_msg += f"Column is missing in training dataframe"

            status = self.is_column_exist(dataframe=test_df)
            if not status:
                validation_error_msg += f"Column is missing in testing dataframe"

            validation_status = len(validation_error_msg) ==0

            if validation_status:
                drift_status = self.detect_dataset_drift(reference_df=train_df,
                                                         current_df=test_df)
                if drift_status:
                    logging.info(f'Dataset drift detected')
                    validation_error_msg = 'Drift detected'
                else:
                    logging.info(f'No dataset drift detected')
            else:
